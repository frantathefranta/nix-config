...
let
  ...
  notmuchPostNew = pkgs.writeShellScript "notmuch-post-new_autotag" ''
    ...
    ${pkgs.notmuch}/bin/notmuch tag +spam -inbox -- 'folder:Junk or folder:"[Gmail]/Spam"' and tag:new
    ${pkgs.notmuch}/bin/notmuch tag +trash -inbox -- 'folder:"Deleted Messages" or folder:"[Gmail]/Trash"' and tag:new

    # Remove 'new' tag
    ${pkgs.notmuch}/bin/notmuch tag -new -- tag:new

    # Tag sent mail with recipient info (for better searching)
    ${pkgs.notmuch}/bin/notmuch tag +sent-to-self -- tag:sent and to:fb@franta.us
  ...
  programs.aerc = {
    ...
    extraAccounts = {
      ...
      combined = {
        ...
        # query-map = "/Users/fbartik/Library/Preferences/aerc/query-map";
        query-map = toString (
    ...
    extraConfig = {
      # Takes care of permissions for aerc/accounts.conf
      general = {
